...
def run_model(task=None, model_name=None, dataset_name=None, config_file=None,
    ...
    # resume training
    if other_args and other_args.get('resume') is not None:
        resume_val = str(other_args.get('resume'))
        cache_dir = f'./libcity/cache/{exp_id}/model_cache'
        
        if resume_val.lower() == 'auto':
            # 自动寻找最新的epoch文件
            if os.path.exists(cache_dir):
                epoch_files = glob.glob(f"{cache_dir}/*_epoch*.tar")
                if epoch_files:
                    epochs = []
                    for f in epoch_files:
                        match = re.search(r'epoch(\d+)\.tar', f)
                        if match:
                            epochs.append(int(match.group(1)))
                    if epochs:
                        resume_epoch = max(epochs)
                        config['epoch'] = resume_epoch  # 这会触发executor加载该epoch
                        # 强制开启训练模式（因为resume意味着要继续训练）
                        train = True
                        config['train'] = True
                    else:
                        raise FileNotFoundError(f"在 {cache_dir} 中未找到有效的epoch文件")
                else:
                    raise FileNotFoundError(f"在 {cache_dir} 中未找到任何checkpoint文件")
            else:
                raise FileNotFoundError(f"Cache目录不存在: {cache_dir}，无法接续训练")
        else:
            # 指定具体epoch数字
            try:
                resume_epoch = int(resume_val)
                
                # 构造预期的文件名（与 save_model_with_epoch 中的命名规则保持一致）
                expected_filename = f"{model_name}_{dataset_name}_epoch{resume_epoch}.tar"
                expected_path = os.path.join(cache_dir, expected_filename)
                
                # 检查文件是否存在
                if not os.path.exists(expected_path):
                    raise FileNotFoundError(
                        f"指定的 epoch {resume_epoch} 的 checkpoint 文件不存在: {expected_path}\n"
                        f"请检查: 1) epoch 数字是否正确; 2) exp_id '{exp_id}' 是否匹配; "
                        f"3) 是否已完成该 epoch 的训练并保存"
                    )
                
                # 文件存在，设置接续训练参数
                config['epoch'] = resume_epoch
                train = True
                config['train'] = True
                logger.info(f"找到指定 checkpoint: {expected_filename}，将从 epoch {resume_epoch} 接续训练")
                
            except ValueError:
                raise ValueError(f"无效的resume参数: {resume_val}，请使用'auto'或数字")

    # logger
    logger = get_logger(config)
    epoch = config.get('epoch', 0)
    logger.info('Begin pipeline, task={}, model_name={}, dataset_name={}, exp_id={}, epoch={}'.
                format(str(task), str(model_name), str(dataset_name), str(exp_id), epoch))
