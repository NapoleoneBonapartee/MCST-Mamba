...
def generate_dataloader(train_data, eval_data, test_data, feature_name,
    ...
    def collator(indices):
        """Optimized collator without deepcopy"""
        batch = Batch(feature_name)
        for item in indices:
            # Use direct assignment instead of deepcopy
            # Data is already in numpy array format from dataset
            batch.append(item)
        return batch

    # Use persistent_workers to avoid worker recreation overhead
